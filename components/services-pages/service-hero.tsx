...
export function ServiceHero({ badge, title, highlight, subtitle, description, primaryBtn, secondaryBtn, annotation }: ServiceHeroProps) {
  return (
        ...
        <h2 className="text-4xl md:text-5xl font-bold text-[#212529] italic mb-12">
          ...
          {subtitle.includes("*") && (
            <span className="relative inline-block not-italic">
              <span className="relative z-10">{subtitle.split("*")[1]}</span>
              <span className="absolute bottom-0 left-0 w-full h-2 bg-[#6EBE45] rounded-full translate-y-2" />
            </span>
          )}
        </h2>

        <p className="text-xl text-gray-600 max-w-2xl mx-auto mb-10">{description}</p>
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-4 mb-8">
          ...
          <Link href={secondaryBtn.href}>
            <Button variant="outline" className="bg-[#F8F9FA] border-none text-[#6EBE45] px-8 py-6 text-lg font-semibold rounded-md group hover:bg-[#F0F0F0]">
              {secondaryBtn.label} <ChevronDown className="ml-2 w-5 h-5 transition-transform group-hover:rotate-180" />
            </Button>
          </Link>
        </div>
        ...
        <div className="absolute right-0 bottom-[-20px] md:right-[10%] md:bottom-[-40px] rotate-[-15deg] hidden md:block">
          <div className="flex flex-col items-center">
            ...
            <p className="text-[#6EBE45] font-bold text-xl leading-none">
              {annotation.line1} <br />
              <span className="text-lg opacity-80 font-medium tracking-tight">{annotation.line2}</span>
            </p>
          </div>
        </div>
      </div>
